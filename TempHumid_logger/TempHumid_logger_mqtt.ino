// Copyright (C) Wayne Geiser, 2018-2023.  All Rights Rreserved.
// email: geiserw@gmail.com
//
// Logs temperature and humidity data from a DHT22 connect to an Arduino to // a ThingSpeak channel and publishes MQTT messages via WiFi
//
// NOTE **************
...
//
// Change log:
//  2018-Oct-29 Changed channel on ThingSpeak to separate this data from 
//              other data being logged to the previous channel
//              Added char definitions for Serial calls to remove warnings 
//              upon compilation
//  2019-Feb-26 Changed WiFi code to loop continuously trying to connect so 
//              it might recover from a power outage.
//  2021-Mar-06 Added Watchdog code in case of hang.
//  2023-Jan-02 Wait around for the WiFi network to connect
//              Rest the arduino if the data can't be updated.  It probably
//              means we lost WiFi without losing power.
//  2023-Nov-12 Added MQTT publishing

#include <stdlib.h>
#include "DHT.h"
#include "SecretStuff.h" // definitions for SSID and PASS (your network name 
                         // and password)
#include <avr/wdt.h>
...
EspMQTTClient client(
	SECRET_SSID,
	SECRET_PASS,
	SECRET_MQTT_BROKER_IP_LOCAL,
	SECRET_MQTT_USER,
	SECRET_MQTT_USER_PASS,
	"BasementTempHumidity",
	1883
);
MQTT_TOPIC "home/basement/familyroom/temphumidity/"
  ...
  if (Serial.find(OK)) {
    ...
    if (!connected) {
	  ...
	  client.publish(MQTT_TOPIC + "error",
	                 "Error #1. Did not connect to WiFi.");
      Error(1);
    }
  }
  else {
	client.publish(MQTT_TOPIC + "error",
	               "Error #2. Did not connect to WiFi.");
    Error(2);
  }
...
// This is called once both WiFi and MQTT are connected
void onConnectionEstablished() {
	// I'm not really sure what should be done here.
	// I don't need to subscribe to anything.
	// Just publish that I've started up, in case anyone cares.
	client.publish("home/basement/familyroom/temphumidity/startup", "ON")
}
...
bool updateTemp(String tempF, String humid) {
  char ERROR[] = "Error";
  char GREATER[] = ">";
  char OK[] = "OK";

  // MQTT Publish the temperature and humidity
  client.publish("home/basement/familyroom/temphumidity/temp", tempF);
  client.publish("home/basement/familyroom/temphumidity/humid", humid);
